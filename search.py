...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisSearch_trial in search_trials:
        ...
        # --- Ending Routine "search_trial" ---
        for thisComponent in search_trialComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('search_trial.stopped', globalClock.getTime(format='float'))
        # Run 'End Routine' code from send_event
        thisExp.addData('grating_angle', grating_angle)
        # check responses
        if key_resp.keys in ['', [], None]:  # No response was made
            key_resp.keys = None
        search_trials.addData('key_resp.keys',key_resp.keys)
        if key_resp.keys != None:  # we had a response
            search_trials.addData('key_resp.rt', key_resp.rt)
            search_trials.addData('key_resp.duration', key_resp.duration)
        # the Routine "search_trial" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
    ...
    # completed 10.0 repeats of 'search_trials'
